...
class dataExtracter:
    def __init__(self, zipName):
        self.allData = {"train": {"spam": [], "ham": []}, "test": {"spam": [], "ham": []}}
    ...
    def readData(self,dataSetName, trainOrTest):
        for trainOrTest in ["train", "test"]:
            with zipfile.ZipFile(os.path.join("./", dataSetName.lower()+"_"+trainOrTest.lower()+".zip"), "r") as zipRef:
                for spamOrHam in ["spam", "ham"]:
                    ...
                    for file in files:
                        self.allData[trainOrTest][spamOrHam.lower()].append(io.TextIOWrapper(zipRef.open(file), encoding='iso-8859-1', newline='').read())

extracted = dataExtracter("project1_datasets")
extracted.readData("enron1")
print(extracted.allData["train"]["spam"][0])
print(extracted.allData["train"]["ham"][0])
